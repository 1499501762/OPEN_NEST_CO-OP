...
# --- Game executable name (inside the game dir) ---
$SteamExe = "Iron Nest Heavy Turret Simulator.exe"
...
$LobbyFile = Join-Path $env:TEMP "open_nest_lobby.txt"

# --- Packaging (scripts/package.ps1) ---
# BepInEx 6 standalone loader zip (Bleeding Edge build #785 required)
$BepInEx6Zip = "$env:USERPROFILE\Downloads\BepInEx-Unity.IL2CPP-win-x64-6.0.0-be.785.zip"
# MelonLoader 0.7.3 standalone loader zip (standard native, NOT BepInEx6 bridge)
$MLLZip = "$env:USERPROFILE\Downloads\MelonLoader.x64.0.7.3.zip"

# --- Derived paths (usually no need to change) ---
$BepInExDir = Join-Path $GameDir "BepInEx"
$GamePluginsDir = Join-Path $BepInExDir ("plugins\")
