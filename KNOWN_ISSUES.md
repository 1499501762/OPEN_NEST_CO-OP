...
### IME 中文输入 — TMP_InputField 根治方案（2026-08-12，待验证）
- **根因（本质）**：**Unity 只有聚焦真实 `TMP_InputField` 时才会唤起系统中文输入法（IME）**。自制 Button+Text 输入框永远无法弹出输入法——无论 `SetIMEEnabled` 还是 patch `OnIMECompositionChanged` 都绕不开「没有 TMP_InputField 就不弹输入法」这一限制。
- **最终方案（根治）**：聊天输入框改为**真实 `TMP_InputField`**（`MakeChatInput`）：
  - 聚焦时 `ActivateInputField()` → Unity 自动唤起系统 IME → 中文可直接输入（游戏自带 CatNameSetting/InputFieldHelper 等 TMP_InputField + InputSystemUIInputModule 同款，已确认在该 IL2CPP 环境可用）。
  - 英文/中文/退格/IME 全部由 TMP_InputField 自己处理，Update 每帧轮询 `text` 同步到 `_chat`（避免 `UnityEvent<string>.AddListener` 的 IL2CPP 委托桥接 CS1503——interop 把 UnityEvent 降级为 AddListener(IntPtr)）。
  - 移除 `OnIMECompositionChanged` patch（不再需要，避免双通道）。
- **聊天面板逻辑（用户要求）**：
  - **未聚焦**：背景/标题/输入框隐藏，**聊天记录常驻显示**（历史消息始终可见），记录文本 `raycastTarget=false` → **鼠标穿透**（不拦截游戏点击）。
  - **聚焦**：显示背景 + 标题 + 记录 + 输入框，激活 TMP_InputField 唤起 IME。
  - 常驻结构不销毁重建（避免输入框失焦）。
- **房间名输入（Idle）**：保留 PollInput 物理字符（英文，非聊天无需中文 IME）。
- **待用户验证**：① 中文 IME 能否弹出并输入；② 未聚焦时聊天记录仍显示 + 鼠标穿透；③ Enter 取消聚焦。
