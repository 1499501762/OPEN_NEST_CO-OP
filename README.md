...
| Version | Highlights |
|---|---|
| **v0.2.1-Alpha-3** (2026-09-13) | **原生 UI 库 + 模组菜单（两个新模组）**：`OpenNestUIKit` —— 自绘 iOS 式声明式菜单框架（自研布局引擎 `UiFlow`/`UiMeasure`/`UiStretch` + `RectMask2D` 裁剪 + 热区命中），**扁平工业风**主题（实色填充 + 1px 程序化描边/细线，零贴图、零布局高度），双端（BepInEx / MelonLoader）共用同一份源码，另出契约程序集 `OpenNestUIKit.API` 供第三方以**软依赖**注册页面（未安装不影响自身加载）；`OpenNestModMenu` —— 独立模组菜单（列表/详情/设置/调试四页），模组**启用禁用 + 上移下移**（顺序落盘 `.order.ini`）、**依赖声明读取**（BepInEx `BepInDependency` / MLL 依赖）与加载顺序匹配、依赖缺失/不兼容提示、第三方 `IModMenuProvider` 接入、界面文案全量本地化、**调试模式开关**（`BepInEx\config\OpenNestModMenu.cfg`）。**联机菜单接入 UIKit 原生控件** + 一键复制 `IP:端口`（`Core/Clipboard.cs`）。**中文输入法（IME）修复四连**：组合文字只在含中日韩字符时写入、显式打开 `Input.imeCompositionMode`（反射）+ IME 上下文兜底、`GCS_RESULTSTR` 是“保持值”不再回填旧文本、跨通道去重（首次候选词不再重复）、发送后不再自动唤起。测试模组新增 `mockcfg`（生成全类型模拟配置文件，验证自动生成的配置控件）与 `imedecide` / `imefake` / `imecomp` 输入法离线回归命令；文档新增 `docs/UI_KIT.md`、`docs/UI_KIT_API.md`、`docs/UI_KIT_SLICE.md`、`docs/UI_KIT_TEST.md`、`docs/MOD_MENU.md`、`docs/NATIVE_UI.md` |
| **v0.2.1-Alpha-2** (2026-09-12) | **LAN co-op** (no Steam lobby required): new TCP transport (`LanTransport`, multi-client, host binds `0.0.0.0`) + UDP room discovery (`LanDiscovery`, `ONCQ`/`ONCR`), **separate tab** in the coop menu (Steam Lobby / LAN Co-op); **identity rule**: SteamID first, otherwise a random **FakeID auto-generated and saved to the config** — **duplicate display names allowed** (identity is the key), same-Steam-account dual instances get a derived unique id automatically; custom player name, one-click **copy/paste of IP:port**, LAN room list with per-room copy/join; CLI: `--lan host` / `--lan join --lanip <ip>` + `dualtest.ps1 -Lan` (verified end-to-end). **All UI text moved to language keys in an external file** (`OpenNestCoop.lang.ini`, hot-reloaded in ~2s, zh/en sections, add your own language, edit text without rebuilding); kick/reject reasons are sent as `@Key` so the receiver shows them in its own language. Also: `NestSync` iron-nest/icon sync switched from 1.5s heartbeat resend to **change detection + cached value** (plus critical-packet protection and late-join resend) |
| **v0.2.1-Alpha-1** (2026-09-12) | **Shell launch-parameter sync** (`ShotSync`, dynamic channel `shotparams`): host broadcasts start/target/flight-time/path-length so both ends fly the identical trajectory; **shell origin root cause fixed** — the origin is the iron-nest map icon `Tactical Map/.../TurretLocation` (= `GunController.firePoint`), now host-authoritative synced (`NestSync` 146); **recon photo angle** derived deterministically from the synced impact point (no more per-end random roll); **typewriter** stall criterion changed to "local reveal actually advancing" (and the game's forced-complete API no longer used — it broke following prints); **client follows host back to main menu / mission select** (`LoadMainMenu` / `EnterBrowsingMap`); **critical-packet protection extended** to dynamic channels + public `RegisterCriticalType` API (fixes mod-side silent packet drops); **mod config file** (`CoopConfig`, standard INI, 2s hot-reload) with cat / record-player / calculate-button switches; new `Artillery Computer Console/Calculate Universal Button` click sync (off by default); `BlockerSync` heartbeat; F10 tool now shows per-interactable sync state |
