...
// ⚠️ 程序集特性在命名空间之外，必须全限定引用 ModMenuInfo（OpenNestModMenu.ModMenuInfo）。
[assembly: MelonInfo(typeof(OpenNestModMenu.MelonModEntry),
                     OpenNestModMenu.ModMenuInfo.Name,
                     OpenNestModMenu.ModMenuInfo.Version,
                     OpenNestModMenu.ModMenuInfo.Author)]
...
public class MelonModEntry : MelonMod
{
    ...
    {
        ModMenuRuntime.Initialize(new MlLogger());
        try
        {
        ...
        {
            MelonLogger.Error($"OpenNestModMenu init failed: {ex}");
        }
    }
