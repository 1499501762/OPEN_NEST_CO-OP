...
///
/// 本接口与 <see cref="NativeUi"/> 门面在本模组内定义（契约部分，不依赖游戏程序集）；
/// **游戏侧实现**放在 <c>Bridge/</c>（能引用 Assembly-CSharp），启动时注册到 <see cref="NativeUi"/>。
...
/// <summary>
/// 原生 UI 桥接注册表 + 静态门面。本模组调用 <c>NativeUi.xxx</c> 即可使用游戏原生 UI 能力，
/// 无需在非桥接代码里引用游戏程序集。游戏侧桥接实现（INativeUiService）在启动时调用 <see cref="Register"/> 注入。
/// 未注册时门面方法安全返回默认值。
/// </summary>
