...
namespace OpenNestModMenu.Logging;

/// <summary>
/// 模组独立文件日志（ModLog）：把高频诊断日志写到**独立 .log 文件**（不附加到 BepInEx/MelonLoader 主日志），
/// 用 StringBuilder 缓冲 + 定时/满额**批量落盘**——降低文件 I/O 次数，且主日志/控制台不再被刷屏（主日志安静 → 帧性能提升）。
///
/// 文件目录：<see cref="Init"/> 时指定（本模组 = &lt;游戏目录&gt;/OpenNestModMenuLogs）；按"类别"分文件（modmenu/loader/config 等，
/// 由 <see cref="CoopLog.RouteToFile"/> 按 key 前缀路由）。惰性：<see cref="Write(string, Func{string})"/> 只在缓冲/落盘时拼接字符串。
/// 驱动：<see cref="Flush(float)"/> 每帧调用（ModMenuBehaviour.Update），达到 1s 间隔或缓冲满才批量写盘。
/// </summary>
public static class ModLog
{
    ...
    public static void Init(string dir)
    {
        try
        {
            ...
            _dir = dir;
            // 每次启动清空旧日志（*.log），新会话从头记（上次会话日志不再残留混淆）。
            try
            {
    ...
    /// <summary>每帧调用（ModMenuBehaviour.Update）：达到 1s 间隔或缓冲满才批量落盘（性能：批量化文件 I/O）。</summary>
