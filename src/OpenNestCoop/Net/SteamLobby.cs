...
public class SteamLobby
{
    ...
    public void CreateLobby(string name, int max)
    {
        ...
        CoopRuntime.LogSource?.LogInfo($"正在创建大厅 (最多 {MaxPlayers} 人)…");
    }
    ...
    public void RefreshBrowser()
    {
        ...
        try
        {
            ...
            CoopRuntime.LogSource?.LogInfo($"发起大厅列表请求 (call={call.m_SteamAPICall})");
        }
        catch (Exception ex)
        {
            CoopRuntime.LogSource?.LogWarning($"刷新大厅异常: {ex.Message}");
        }
    }
    ...
    private void OnLobbyCreated(LobbyCreated_t p, bool ioFailure)
    {
        ...
        {
            CoopRuntime.LogSource?.LogError($"创建大厅失败: {p.m_eResult}");
            LastError = $"创建大厅失败: {p.m_eResult}";
            Left?.Invoke();
            return;
        }
        ...
        CoopRuntime.LogSource?.LogInfo($"大厅创建成功: {LobbyID}");
        Entered?.Invoke();
    }
    ...
    private void OnLobbyEntered(LobbyEnter_t p, bool ioFailure)
    {
        ...
        {
            CoopRuntime.LogSource?.LogWarning($"加入大厅失败: 响应码 {p.m_EChatRoomEnterResponse}");
            LastError = $"加入大厅失败: 响应码 {p.m_EChatRoomEnterResponse}";
            Left?.Invoke();
            return;
        }
        LobbyID = (CSteamID)p.m_ulSteamIDLobby;
        IsHost = false;
        MaxPlayers = SteamMatchmaking.GetLobbyMemberLimit(LobbyID);
        CoopRuntime.LogSource?.LogInfo($"已加入大厅: {LobbyID}");
        Entered?.Invoke();
    }
    ...
    private void OnLobbyList(LobbyMatchList_t p, bool ioFailure)
    {
        ...
        CoopRuntime.LogSource?.LogInfo($"大厅列表回调: 收集到 {_pendingIds.Count} 个大厅");
    ...
    public void PollPendingLobbyList()
    {
        ...
        try
        {
            ...
            _pendingIds.Clear();
            CoopRuntime.LogSource?.LogInfo($"大厅列表刷新: {Browser.Count} 个房间");
        }
        catch (Exception ex) { CoopRuntime.LogSource?.LogWarning($"大厅列表填充异常: {ex.Message}"); }
    ...
    {
        CoopRuntime.LogSource?.LogInfo($"收到好友加入请求，自动加入大厅 {p.m_steamIDLobby}");
