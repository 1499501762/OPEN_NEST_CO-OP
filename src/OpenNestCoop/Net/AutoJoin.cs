    ...
    public static void ParseCommandLine()
    {
        try
        {
            ...
            for (int i = 0; i < args.Length; i++)
            {
                ...
                else if (a.Equals("--lag", StringComparison.OrdinalIgnoreCase) && i + 1 < args.Length)
                {
                    // 网络延迟模拟（本地测试）：基础单向延迟 ms
                    if (int.TryParse(args[++i], out int lag)) LagMs = Math.Max(0, lag);
                }
                else if (a.Equals("--lagjitter", StringComparison.OrdinalIgnoreCase) && i + 1 < args.Length)
                {
                    // 网络延迟波动范围 ±ms
                    if (int.TryParse(args[++i], out int j)) JitterMs = Math.Max(0, j);
                }
