...
public static class AutoJoin
{
    ...
    public static void ParseCommandLine()
    {
        try
        {
            ...
            if (any)
                CoopRuntime.LogSource?.LogInfo($"[AutoJoin] 参数: host={WantHost} join={WantJoin} localHost={WantLocalHost} localJoin={WantLocalJoin} port={LocalPort}");
        }
        catch (Exception ex)
        {
            CoopRuntime.LogSource?.LogWarning($"[AutoJoin] 解析参数失败: {ex.Message}");
        }
    }
    ...
    public static void TryStart(NetManager net)
    {
        ...
        if (WantLocalHost || WantLocalJoin)
        {
            ...
            if (WantLocalHost)
            {
                ...
                net.CreateLobby();
                CoopRuntime.LogSource?.LogInfo($"[AutoJoin] 本地模式 host（端口 {LocalPort}）");
            }
            else
            {
                ...
                {
                    _handled = true;
                    CoopRuntime.LogSource?.LogInfo($"[AutoJoin] 本地模式 client 已连接（端口 {LocalPort}）");
                }
                else
                {
                    CoopRuntime.LogSource?.LogInfo("[AutoJoin] 本地 client 连接失败，重试…");
                }
            }
            return;
        }
        ...
        if (WantHost)
        {
            ...
            _hostTriggered = true;
            net.CreateLobby();
            CoopRuntime.LogSource?.LogInfo("[AutoJoin] 自动建房触发");
        }
        else if (WantJoin)
        {
            if (!_pausedForJoin) { PauseForJoin(); _pausedForJoin = true; }
    ...
    private static void PauseForJoin()
    {
        try
        {
            ...
            CoopRuntime.LogSource?.LogInfo("[AutoJoin] 自动加入：已暂停游戏（加入完成后恢复）");
        }
    ...
    public static void ResumeIfPaused()
    {
        ...
        try
        {
            if (PauseManager.IsPaused)
            {
                ...
                CoopRuntime.LogSource?.LogInfo("[AutoJoin] 自动加入完成：已恢复游戏");
            }
        }
    ...
    public static void OnHostEntered(NetManager net)
    {
        ...
        catch (Exception ex) { CoopRuntime.LogSource?.LogWarning($"[AutoJoin] 写 lobby id 失败: {ex.Message}"); }
    }
    ...
    private static void TryJoinFromFile(NetManager net)
    {
        try
        {
            ...
            {
                CoopRuntime.LogSource?.LogWarning($"[AutoJoin] 未找到 lobby 文件 {LobbyFile}，等待重试");
            ...
            {
                CoopRuntime.LogSource?.LogWarning($"[AutoJoin] lobby 文件内容无效: '{text}'");
                _handled = false;
                return;
            }
            net.JoinLobby(new LobbyInfo { Id = id });
            CoopRuntime.LogSource?.LogInfo($"[AutoJoin] 自动加入大厅 {id}");
        }
        catch (Exception ex)
        {
            CoopRuntime.LogSource?.LogWarning($"[AutoJoin] 加入失败: {ex.Message}");
            _handled = false;
        }
    }
}
