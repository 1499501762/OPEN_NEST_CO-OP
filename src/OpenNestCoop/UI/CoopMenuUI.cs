...
public static class CoopMenuUI
{
    ...
    private static bool _inputPassword;
    private static bool _passwordDialog;
    private static Net.LobbyInfo _pendingJoin;
    private static string _joinPassword = "";
    ...
    private static float DrawIdle(NetManager net, float x, float cy, float w, float py)
    {
        ...
        if (_roomPassword.Length == 0 && net.PendingPassword.Length > 0) _roomPassword = net.PendingPassword;
        ...
        Label(x, cy, 80, 20, "房间密码(可选)"); cy += 24;
        DrawInputBox(x, cy, w - 56, 24, _roomPassword.Length > 0 ? new string('*', _roomPassword.Length) : "");
        if (Button(x + w - 52, cy, 46, 24, (_typing && _inputPassword) ? "停" : "✎"))
        {
            _inputPassword = true;
            _typing = !_typing;
        }
        ...
        if (net.LastLobbyId != 0 && Button(x + w - 150, cy, 140, 26, "重连上次房间")) net.JoinRecentLobby();
        cy += 34;
        ...
        else
        {
            ...
            for (int i = start; i < end; i++)
            {
                ...
                string verTag = string.IsNullOrEmpty(info.Version)
                    ? "<color=#f66>旧版?</color>"
                    : (info.Version == OpenNestCoop.Core.NetConfig.Version
                        ? $"<color=#8f8>v{info.Version}</color>"
                        : $"<color=#f66>v{info.Version}</color>");
                var title = info.IsFull
                    ? $"{info.Name} {lockTag}{loaderTag} <color=#888>{info.Players}/{info.MaxPlayers} 已满</color> {verTag}{ownerTag}"
                    : $"{info.Name} {lockTag}{loaderTag} <color=#8f8>{info.Players}/{info.MaxPlayers}</color> {verTag}{ownerTag}";
        ...
        {
            cy += 6;
            Label(x, cy, w, 20, $"<color=#fc0>{CoopLoc.PasswordRequired}: {(_pendingJoin != null ? _pendingJoin.Name : "")}</color>"); cy += 26;
            DrawInputBox(x, cy, w - 56, 24, _joinPassword.Length > 0 ? new string('*', _joinPassword.Length) : "");
            if (Button(x + w - 52, cy, 46, 24, (_typing && _passwordDialog) ? "停" : "✎"))
                _typing = !_typing;
            cy += 30;
            if (Button(x, cy, w / 2 - 6, 26, "确定")) ConfirmPasswordJoin();
            if (Button(x + w / 2 + 6, cy, w / 2 - 6, 26, "取消"))
            {
    ...
    private static void ConfirmPasswordJoin()
    {
        var net = CoopRuntime.Net;
        ...
        {
            _joinPassword = "";
            net.LastError = OpenNestCoop.UI.CoopLoc.PasswordError;
            return;
        }
    ...
    {
        Label(x, cy, w - 140, 22, $"房间: <b>{net.PendingLobbyName}</b>");
        Label(x + w - 130, cy, 64, 22, $"<color=#8f8>v{OpenNestCoop.Core.NetConfig.Version}</color> <color=#aaa>{OpenNestCoop.Core.NetConfig.LoaderName}</color>");
        ...
        Label(x, cy, w, 20, $"{OpenNestCoop.UI.CoopLoc.RoomPwd}: {(hasPwd ? OpenNestCoop.UI.CoopLoc.PwdHas : OpenNestCoop.UI.CoopLoc.PwdNone)}");
        cy += 24;
    ...
    private static void Append(char c)
    {
        if (IsRoomMode())
        {
            if (_passwordDialog) { if (_joinPassword.Length < 40) _joinPassword += c; }
            else if (_inputPassword) { if (_roomPassword.Length < 40) _roomPassword += c; }
            else if (_roomName.Length < 40) _roomName += c;
        }
        else
        {
    ...
    private static void Backspace()
    {
        if (IsRoomMode())
        {
            if (_passwordDialog) { if (_joinPassword.Length > 0) _joinPassword = _joinPassword.Substring(0, _joinPassword.Length - 1); }
            else if (_inputPassword) { if (_roomPassword.Length > 0) _roomPassword = _roomPassword.Substring(0, _roomPassword.Length - 1); }
            else if (_roomName.Length > 0) _roomName = _roomName.Substring(0, _roomName.Length - 1);
        }
        else
        {
    ...
    public static void TrackClick()
    {
        ...
        try
        {
            ...
            if (!_mouseDiagDone)
            {
                _mouseDiagDone = true;
                var kb = UnityEngine.InputSystem.Keyboard.current;
                CoopRuntime.LogSource?.LogInfo($"[Input] Mouse.current={(mouse != null ? "OK" : "NULL")} Keyboard.current={(kb != null ? "OK" : "NULL")}");
            }
        }
        catch { }
