...
public class CoopBehaviour : MonoBehaviour
{
    ...
    public void Update()
    {
        ...
        {
            _updatedOnce = true;
            CoopRuntime.LogSource?.LogInfo("[CoopBehaviour] Update 开始被调用");
        ...
        {
            CoopRuntime.LogSource?.LogError($"[CoopBehaviour] Update 异常: {ex}");
        }
    }
    ...
    private static void UnpauseForCoop()
    {
        ...
        CoopRuntime.LogSource?.LogInfo("[CoopBehaviour] 已移除失焦暂停（PauseOnFocusLoss=false, timeScale=1）");
    }
}
