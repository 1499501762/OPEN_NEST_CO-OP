...
public static class MapSync
{
    ...
    public static void OnAdd(ulong from, byte[] data)
    {
        ...
        try
        {
            ...
            if (net.IsHost && id == 0)
            {
                ...
                // 离散状态（放置标记）必须 reliable：边沿触发、无周期重发，丢了对端永久缺失
    ...
    public static void OnRemove(ulong from, byte[] data)
    {
        ...
        try
        {
            ...
            int id = r.GetInt();
            ApplyRemove(id);
            // 离散状态（移除标记）必须 reliable：丢了对端标记永久残留
            if (net.IsHost && net.Roster != null)
                foreach (var p in net.Roster)
                    if (!p.IsLocal && p.SteamId != from)
                        net.Transport.Send(p.SteamId, data, true);
    ...
    public static void OnClearAll(ulong from, byte[] data)
    {
        ...
        {
            ApplyClearAll();
            // 离散状态（清空标记）必须 reliable：丢了对端标记永久残留
            if (net.IsHost && net.Roster != null)
                foreach (var p in net.Roster)
                    if (!p.IsLocal && p.SteamId != from)
                        net.Transport.Send(p.SteamId, data, true);
    ...
    private static MapMarkerPlacer GetPlacer()
    {
        ...
        if (_placer == null)
        {
            ...
            if (_placer == null)
                CoopRuntime.LogSource?.LogWarning("MapSync: MapMarkerPlacer not found (map marker sync disabled)");
        }
        return _placer;
    }
}
