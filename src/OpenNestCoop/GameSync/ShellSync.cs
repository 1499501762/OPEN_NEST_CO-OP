...
public sealed class ShellSync : ISyncedModule
{
    ...
    public void Tick(float dt)
    {
        ...
        try
        {
            ...
            {
                if ((++_scanLog % 20) == 1)
                    CoopRuntime.LogSource?.LogInfo("[ShellSync] 未找到 CylinderShellSelector（场景无此组件或未启用）");
                return;
            }
            if ((++_scanLog % 10) == 1)
                CoopRuntime.LogSource?.LogInfo($"[ShellSync] scan cyls={cyls.Length} firstCsv='{SnapshotCsv(cyls[0])}'");
            for (int i = 0; i < cyls.Length; i++)
            {
                ...
                CoopRuntime.LogSource?.LogInfo($"[ShellSync] host send idx={i} csv='{csv}'");
            }
        }
    ...
    public void OnPacket(ulong from, byte[] data)
    {
        ...
        try
        {
            ...
            if ((++_recvDiag % 3) == 1)
                CoopRuntime.LogSource?.LogInfo($"[ShellSync] recv idx={idx} csv='{csv}' isHost={net.IsHost}");
            ApplyCsv(idx, csv);
        }
    ...
    private static string SnapshotCsv(CylinderShellSelector cyl)
    {
        ...
        try
        {
            if (cyl.bullets == null)
            {
                CoopRuntime.LogSource?.LogInfo("[ShellSync] cyl.bullets 为 null（无法检测弹种变化）");
                return sb.ToString();
            }
            ...
            if (cnt > 0 && (nullBp > 0 || nullDef > 0))
                CoopRuntime.LogSource?.LogInfo($"[ShellSync] SnapshotCsv cnt={cnt} nullBp={nullBp} nullDef={nullDef} csv='{sb}'");
        }
        catch (Exception ex) { CoopRuntime.LogSource?.LogWarning($"[ShellSync] SnapshotCsv 异常: {ex.Message}"); }
        return sb.ToString();
    }
    ...
    private static void ApplyCsv(byte idx, string csv)
    {
        try
        {
            ...
            try
            {
                ...
                {
                    CoopRuntime.LogSource?.LogInfo($"[ShellSync] 装填中跳过弹种应用 idx={idx} csv='{csv}'");
                    return;
                }
            }
            catch { }
            ...
            if (allNull)
            {
                try
                {
                    ...
                    CoopRuntime.LogSource?.LogInfo($"[ShellSync] applied 全空 idx={idx}（主机权威清空跟随）");
                }
                catch (Exception ex) { CoopRuntime.LogSource?.LogWarning($"[ShellSync] 全空应用失败: {ex.Message}"); }
                return;
            }
            // 诊断：打印本机 shellPrefabs 的 ShellId + prefab 名列表（排查 prefab 匹配失败）
            {
                ...
                CoopRuntime.LogSource?.LogInfo($"[ShellSync] 诊断 idx={idx} csv='{csv}' prefabCnt={(cyl.shellPrefabs == null ? -1 : cyl.shellPrefabs.Count)} prefabIds='{string.Join(",", ids)}' names='{string.Join("|", names)}'");
            }
            ...
            if (!allMatched)
            {
                CoopRuntime.LogSource?.LogWarning($"[ShellSync] 部分 prefab 未匹配，跳过应用 idx={idx} csv='{csv}'");
                return;
            }
            try
            {
            ...
            {
                CoopRuntime.LogSource?.LogWarning($"[ShellSync] ReplaceAllShells 失败，回退 rebuild: {ex.Message}");
