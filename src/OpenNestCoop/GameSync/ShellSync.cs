...
public sealed class ShellSync : ISyncedModule
{
    ...
    public void Tick(float dt)
    {
        ...
        try
        {
            // 双通道获取 cyl：① FindObjectsOfType（includeInactive，常规）；② turret.guns→reload.cylinderShellSelector
            // （兜底——主机端 CylinderShellSelector 场景不可见时 FindObjectsOfType 返回空 → 弹匣不同步根因）。
            var cyls = new List<CylinderShellSelector>();
            var fo = UnityEngine.Object.FindObjectsOfType<CylinderShellSelector>(true);
            if (fo != null)
                foreach (var c in fo) if (c != null && !cyls.Contains(c)) cyls.Add(c);
            try
            {
                var turret = TurretController.Instance;
                if (turret != null && turret.guns != null)
                    for (int gi = 0; gi < turret.guns.Count; gi++)
                    {
                        var g = turret.guns[gi];
                        if (g == null) continue;
                        var rl = g.artilleryReloadController;
                        if (rl == null) continue;
                        var sc = rl.cylinderShellSelector;
                        if (sc != null && !cyls.Contains(sc)) cyls.Add(sc);
                    }
            }
            catch { }
            if (cyls.Count == 0)
            ...
            if ((++_scanLog % 10) == 1)
                CoopLog.Debug("ShellSync.scan", () => $"[ShellSync] scan cyls={cyls.Count} firstCsv='{SnapshotCsv(cyls[0])}'");
            for (int i = 0; i < cyls.Count; i++)
            {
                var cyl = cyls[i];
                if (cyl == null) continue;
                // ⚠️ 2026-08-22：弹夹标识改用 **关联 GunController 的 Transform 路径**（含 "Gun System Left/Right"，
                // 场景静态、两端一致——同 ButtonClickSync 定位）。不用 FindObjectsOfType 索引（两端顺序可能不同）
                // 也不用 turret.guns 索引（两端顺序可能不同 → 左右炮弹夹错位，"客机左炮转轮弹夹不同步"根因）。
                string gunKey = GunKeyOf(cyl);
                if (string.IsNullOrEmpty(gunKey))
                {
                    CoopLog.Debug("ShellSync.noGun", () => $"[ShellSync] cyl no gun match, skip idx={i} path='{PathOf(cyl.transform)}'");
                    continue;
                }
                ...
                CoopLog.Debug("ShellSync.send", () => $"[ShellSync] host send gun='{gunKey}' csv='{csv}'", 1f);
            }
        }
        catch (Exception ex) { CoopRuntime.LogSource?.LogWarning($"ShellSync Tick: {ex.Message}"); }
    }

    /// <summary>取 CylinderShellSelector 关联 GunController 的 Transform 路径（含 "Gun System Left/Right"，
    /// 场景静态两端一致）。通过 artilleryReloadController → TurretController.guns 匹配（按 Pointer，不看索引顺序）。</summary>
    private static string GunKeyOf(CylinderShellSelector cyl)
    {
        try
        {
            ...
            if (reload == null)
            {
                CoopLog.Debug("ShellSync.noReload", () => $"[ShellSync] cyl.artilleryReloadController is null path='{PathOf(cyl.transform)}'");
                return "";
            }
            for (int i = 0; i < turret.guns.Count; i++)
            {
                ...
                if (g.artilleryReloadController.Pointer == reload.Pointer)
                {
                    ...
                    return "gun" + i; // 兑底：拿不到路径用索引
                }
            }
        }
        catch (Exception ex) { CoopRuntime.LogSource?.LogWarning($"ShellSync GunKeyOf: {ex.Message}"); }
        return "";
    }
    ...
    private static CylinderShellSelector FindCylByGunKey(string gunKey)
    {
        try
        {
            ...
            for (int i = 0; i < turret.guns.Count; i++)
            {
                ...
                return sel; // 可能为 null（BepInEx interop 仅暴露 cylinderShellSelector，cylinderSelector/selector 不存在）
            }
        }
        catch (Exception ex) { CoopRuntime.LogSource?.LogWarning($"ShellSync FindCylByGunKey: {ex.Message}"); }
        return null;
    }
    ...
    public void OnPacket(ulong from, byte[] data)
    {
        ...
        try
        {
            ...
            if ((++_recvDiag % 3) == 1)
                CoopLog.Debug("ShellSync.recv", () => $"[ShellSync] recv gun='{gunKey}' csv='{csv}' isHost={net.IsHost}");
            ApplyCsv(gunKey, csv);
        }
    ...
    private static void ApplyCsv(string gunKey, string csv)
    {
        try
        {
            // ⚠️ 2026-08-22：按 GunController 路径（含 Left/Right）定位弹夹——两端稳定。
            // 不依赖 FindObjectsOfType（主机端 CylinderShellSelector 场景不可见 → 弹匣不同步根因），
            // 改为从 turret.guns[i].artilleryReloadController.cylinderShellSelector 直接取。
            CylinderShellSelector cyl = FindCylByGunKey(gunKey);
            if (cyl == null)
            {
                CoopLog.Debug("ShellSync.applyNoCyl", () => $"[ShellSync] no cyl for gun='{gunKey}' (maybe not yet loaded)");
                return;
            }
            ...
            try
            {
                ...
                {
                    CoopLog.Debug("ShellSync.skipReload", () => $"[ShellSync] reloading, skip shell apply gun='{gunKey}' csv='{csv}'");
                    return;
                }
            }
            catch { }
            ...
            if (allNull)
            {
                try
                {
                    ...
                    CoopRuntime.LogSource?.LogInfo($"[ShellSync] applied all-empty gun='{gunKey}' (host-authoritative clear follow)");
                }
            ...
            // 诊断：打印本机 shellPrefabs 的 ShellId + prefab 名列表（排查 prefab 匹配失败）
            {
                ...
                CoopRuntime.LogSource?.LogInfo($"[ShellSync] diag gun='{gunKey}' csv='{csv}' prefabCnt={(cyl.shellPrefabs == null ? -1 : cyl.shellPrefabs.Count)} prefabIds='{string.Join(",", ids)}' names='{string.Join("|", names)}'");
            }
            ...
            if (!allMatched)
            {
                CoopRuntime.LogSource?.LogWarning($"[ShellSync] some prefabs unmatched, skip apply gun='{gunKey}' csv='{csv}'");
                return;
            }
            try
            {
                ...
                CoopRuntime.LogSource?.LogInfo($"[ShellSync] applied gun='{gunKey}' csv='{csv}' (ReplaceAllShells)");
            }
            catch (Exception ex)
            {
