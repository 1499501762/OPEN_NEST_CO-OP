        ...
        try
        {
            ...
            if (!_debugDumped)
            {
                ...
                for (int i = 0; i < map.transform.childCount && shown < 6; i++)
                {
                    ...
                    CoopRuntime.LogSource?.LogInfo($"[MapTokenSync] DEBUG token='{t.name}' comps=[{comps}] kids=[{kids}]");
                }
            }
            ...
            if (++_pathFrame % 60 == 0)
            {
                ...
                CoopRuntime.LogSource?.LogInfo($"[MapTokenSync] token ids=[{paths}]");
            }
            ...
            {
                if ((++_sendLog % 40) == 1)
                    CoopRuntime.LogSource?.LogInfo($"[MapTokenSync] scan tokens={scanned}");
                return;
            }
            ...
            if ((++_sendLog % 20) == 1)
            {
                ...
                CoopRuntime.LogSource?.LogInfo($"[MapTokenSync] ids=[{ids}]");
            }
            CoopRuntime.LogSource?.LogInfo($"[MapTokenSync] send changed={changed.Count} host={net.IsHost}");
        }
