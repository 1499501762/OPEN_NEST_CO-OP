...
public sealed class StateSnapshotSync : ISyncedModule
{
    ...
    public static void RequestSnapshot()
    {
        ...
        try
        {
            ...
            CoopRuntime.LogSource?.LogInfo("[StateSnapshot] 请求补发快照 → 主机");
        }
    ...
    public void OnPacket(ulong from, byte[] data)
    {
        ...
        try
        {
            ...
            if (msgType == 31)
            {
                ...
                if (net.IsHost)
                {
                    CoopRuntime.LogSource?.LogInfo($"[StateSnapshot] 收到补发请求 → {from}");
                    OnLateJoin(from);
                    ...
                    catch (Exception ex) { CoopRuntime.LogSource?.LogWarning($"[StateSnapshot] ReloadSync 补发: {ex.Message}"); }
                }
                return;
            }
            int n = r.GetByte();
            for (int i = 0; i < n; i++)
            {
                ...
                if (!applied)
                    CoopRuntime.LogSource?.LogWarning($"[StateSnapshot] 未注册模块 '{name}' 忽略");
            }
        }
