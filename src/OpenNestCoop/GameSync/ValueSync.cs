...
public static class ValueSync
{
    ...
    public static void Remove(string id) => _bindings.Remove(id);
    ...
    public static void Tick(float dt)
    {
        ...
        if (_diagTimer >= 1f)
        {
            ...
            foreach (var b in _bindings.Values)
            {
                ...
                {
                    try { CoopRuntime.LogSource?.LogInfo(b.Diag()); } catch { }
                }
            }
        }
    ...
    private static void HostTick(NetManager net, bool heartbeat, bool highFreqOnly)
    {
        foreach (var b in _bindings.Values)
        {
            ...
            if (b.SenderWhenBusy && (b.IsBusy == null || !b.IsBusy())) continue;
            float cur = GetValue(b);
            ...
            b.ForceNext = false;
            // 只在 值变化 / edge 状态变化 / 周期心跳 / 强制广播 时发送（NoHeartbeat 绑定心跳时跳过）。
            // 心跳每 2s 一次（不刷屏），保证新加入/重连客户端拿到完整状态（初始对齐）。
            bool changed = (forced || (!heartbeat || !b.NoHeartbeat)) && (!b.HSet || Delta(cur, last) >= b.Deadzone || edgeCur != edgeLast || forced);
            if (!changed) continue;
            MarkHost(b);
            // E 分级：高频 tick（30Hz Lever/Gear）→ unreliable（容忍丢，连续值）；低频 + 心跳 → reliable（2s 全量保底对齐）
            SendState(net, b, cur, highFreqOnly);
        }
    }
    ...
    private static void ClientTick(NetManager net, bool highFreqOnly)
    {
        foreach (var b in _bindings.Values)
        {
            ...
            if (b.SenderWhenBusy && (b.IsBusy == null || !b.IsBusy())) continue; // 转速：非操作方不上行
    ...
    private static void SendState(NetManager net, Binding b, float v, bool unreliable)
    {
        ...
        if ((++_stateSendLog % 40) == 1)
            CoopLog.Debug("ValueSync.stateSend", () => $"[ValueSync] state send kind={b.Kind} id='{b.Id}' v={v:0.###} drag={(b.IsBusy != null && b.IsBusy() ? 1 : 0)} unrel={unreliable}");
    }
    ...
    private static void SendCmd(NetManager net, Binding b, float v, bool unreliable)
    {
        ...
        if ((++_cmdSendLog % 40) == 1 && b.Id != null && b.Id.IndexOf("Spur Gear", System.StringComparison.OrdinalIgnoreCase) >= 0)
            CoopLog.Debug("ValueSync.cmdSend", () => $"[ValueSync] cmd send '{b.Id}' v={v:0.###} unrel={unreliable}");
    }
