        ...
        try
        {
            ...
            for (int i = 0; i < n; i++)
            {
                ...
                try
                {
                    ...
                    // ⚠️ 2026-08-15 主机权威：客户端收到主机状态广播总是 SetState 对齐 stateIndex +
                    // UpdateAllAdvanceButtons（按钮 active 随权威状态机刷新）——否则客户端装填状态机漂移，
                    // 发射组件（Button Dispencer/ArmingLever/Charge Rammer）active 错乱 → 点击排队 (inactive)
                    // 无限堆积 → 左炮卡死（发射台 Switch 同类问题，主机权威数值对账）。
                    ApplySnapshot(idx, st, ch, true);
