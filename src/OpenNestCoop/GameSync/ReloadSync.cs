...
public static class ReloadSync
{
    ...
    internal sealed class GunReload
    {
        ...
        /// <summary>上次补激活链时的 currentSelectedCharges（2026-08-23：变化 → ActivateNextChargeButtonIfValid 补激活）。</summary>
        public int LastActivateCh = -1;
        // 本地 currentStateIndex 最近变化时间（装填动画/推进进行中检测；动画期间不写远端索引防打断）
        // ⚠️ V1 死代码：只写不读（全仓无读取点），已注释。
        // public float LastStateChange;
        public bool Applying;
        /// <summary>客户端本地装填推进最近时间（2026-08-16 动画保护窗口：此期间主机广播 SetState 不覆盖本地动画，\n
        /// 防装填动画回跳/不同步；超窗仍不一致则 SetState 对齐防按钮卡死）。</summary>
        public float LastLocalChange = -10f;
    }
    ...
    private static int _recvStateLog;
    private static int _cmdLog;
    private static int _upLog;
    ...
    public static void Tick(float dt)
    {
        ...
        // ⚠️ 2026-08-23 **移除** Tick 补激活链（ActivateNextChargeButtonIfValid）——
        // 它开局就触发（LastActivateCh=-1 → ch=0 变化 → 激活 Button 1），导致主机开局 Button 1 被提前激活
        // （"选装药拉杆一开始就激活"元凶），掩码同步又同步给客机 → 双端开局都激活（用户期望开局全不激活，
        // 装填阶段指示器按次序亮，第三阶段才拉装药）。
        // Button 激活同步改由 ChargeButtonSync（MsgType 143）主机权威掩码同步负责（事件/权威驱动，开局不误激活）。
        // apply 后的 ActivateNextChargeButtonIfValid 保留（仅选药量事件触发，逐档跟随合理）。
    ...
    /// <summary>周期全量广播计时（开局/漂移兜底：客户端始终对齐主机权威 st/ch，
    /// 解决“开局初始装药量不同步”——仅变化广播覆盖不了两端初始值差异）。</summary>
    private static float _heartbeat;
    private const float HeartbeatInterval = 5f; // 2026-08-17: 2s→5s 降频（心跳全量广播降网络/对端处理，减少 FPS 损失）
    ...
    {
        // ⚠️ 2026-08-23 诊断：无条件记录 OnLoadChargesPressed 是否被调用 + block 原因（powder load 日志为零，
        // 确认是游戏不调用该方法，还是被防环 block）。
        CoopRuntime.LogSource?.LogInfo($"[ReloadSync] powder load ENTER appPowder={IsApplyingPowder} appClick={ButtonClickSync.IsApplyingClick} powder={powder != null}");
        if (IsApplyingPowder || ButtonClickSync.IsApplyingClick) return; // 应用远端时不转发（防环）
        var net = CoopRuntime.Net;
        if (net == null || powder == null) return;
        if (net.State != SessionState.Hosting && net.State != SessionState.Joined) return;
        int idx = IndexOfPowder(powder);
        if (idx < 0) { CoopRuntime.LogSource?.LogInfo("[ReloadSync] powder load idx NOT FOUND"); return; }
        try
        {
            var w = NetProtocol.Begin(MsgType.PowderEvent);
            w.Put((byte)idx);
    ...
    public static void OnPowderEvent(byte[] data)
    {
        ...
        try
        {
            ...
            try
            {
                ...
                {
                    // ⚠️ 2026-08-23 演变：
                    //   a) 模拟点击 btn.OnClickDown → 按钮 1-based 绑定 OnChargeButtonPressed(N)，与发送方 0-based 差 1 → 反复跳；
                    //   b) 直接调 OnChargeButtonPressed(chargeIndex)（数值一致）→ 但不触发按钮完整点击链路
                    //      （EnableLoadChargesAfterDelay coroutine）→ 对端 Charge Rammer 不启用（inactive）→ 下药无动画。
                    //   c) 最终：模拟点击（触发完整链路：动画 + coroutine 启用 Charge Rammer）**并强制设回 chargeIndex**
                    //      （发送方游戏 OnChargeButtonPressed(__0) 后 currentSelectedCharges=__0=chargeIndex，已验证）→
                    //      两端数值一致 + 对端 Charge Rammer 启用 → 下药动画可复现。
                    var btn = FindDispencerButton(g.Powder, chargeIndex);
                    bool btnActive = btn != null && btn.isActive;
                    if (btnActive) { btn.OnClickDown(); btn.OnClickUp(); }
                    else g.Powder.OnChargeButtonPressed(chargeIndex); // 兜底：按钮 inactive 直接触发逻辑
                    // ⚠️ 2026-08-23 修复：**不再强制 currentSelectedCharges = chargeIndex**！
                    // 日志证实游戏 OnChargeButtonPressed 参数是 **0-based**（Button Dispencer (N) → N-1），
                    // 方法内部 currentSelectedCharges = 药包数（= index+1，Button 1 → 1 个药包）。
                    // 原强制覆盖成 chargeIndex 造成差 1：拉 Button 1 → 0 个药 → "没下装药"；
                    // 拉 Button 2 → 1 个药 → "才下 1 个装药"（与用户反馈完全吻合）。
                    // 让游戏自己设 currentSelectedCharges；下方诊断打印实际值验证两端一致。
                    int actualCh = -1;
                    try { actualCh = g.Powder.currentSelectedCharges; } catch { }
                    // ⚠️ 2026-08-23：对端应用选药量后**主动补激活链**（ActivateNextChargeButtonIfValid）——
                    // Button Dispencer 是逐档激活（选 N 档激活到 N），客机若激活链断（选 2 档后 Button 3
                    // 未激活 allActive 停 110000）→ 点不了 3 档 → "客机锁定 3-6"死锁。补激活让客机按钮跟随。
                    try { g.Powder.ActivateNextChargeButtonIfValid(); } catch { }
                    // ⚠️ 2026-08-23：不调 ApplyInventoryAvailabilityToUI——它会按库存刷新按钮 active，
                    // 覆盖游戏"逐档激活"链 → 客机 Button 3-6 错乱锁定。按钮 active 由游戏选药量流程驱动。
                    // ⚠️ 2026-08-23 诊断：全部 Button Dispencer 的 active 掩码——定位"客机锁定 3-6"根因
                    // （按钮 active 由库存/档位/状态机哪层决定）。
                    string act = "";
                    try
                    {
                        var dcs = g.Powder.chargeButtons;
                        if (dcs != null)
                            for (int di = 0; di < dcs.Count; di++)
                            {
                                var dt = dcs[di];
                                bool da = false;
                                try { da = dt != null && dt.isActive; } catch { }
                                act += (act.Length > 0 ? "," : "") + (da ? "1" : "0");
                            }
                    }
                    catch { }
                    // ⚠️ 2026-08-23 诊断：Button Dispencer 应用后 toggler（高亮）状态——定位"错误锁定"
                    // （按钮不可交互 或 高亮错乱/差 1）。Button Dispencer 实际 4 个 AnimatorBoolToggler。
                    string tg = "";
                    if (btn != null)
                    {
                        try
                        {
                            var tgs = btn.GetComponents<AnimatorBoolToggler>();
                            for (int ti = 0; ti < tgs.Length; ti++)
                            {
                                bool b = false;
                                try { b = tgs[ti].GetBool(); } catch { }
                                tg += (tg.Length > 0 ? "," : "") + (b ? "1" : "0");
                            }
                        }
                        catch { }
                    }
                    CoopRuntime.LogSource?.LogInfo($"[ReloadSync] powder select apply idx={idx} charge={chargeIndex} ch={actualCh} btn={(btn != null)} active={btnActive} allActive=[{act}] tg=[{tg}]");
                }
                else if (ev == 2)
                {
    ...
    public static void OnState(byte[] data)
    {
        ...
        try
        {
            ...
            r.GetByte(); // 跳过消息类型
            int n = r.GetByte();
            int applyState = r.GetByte(); // 0=常规广播；1=中途加入全量（含药包实体 UI 刷新）
            var guns = ResolveGuns();
            for (int i = 0; i < n; i++)
            {
                int idx = r.GetByte();
                ...
                try
                {
                    ...
                    // ⚠️ 2026-08-16 主机权威 + 动画保护：常规广播（applyState=0）且客户端本地刚推进装填（窗口内）
                    // 不 SetState 覆盖（防装填动画回跳）；中途加入（applyState=1）或超窗仍不一致 → SetState 对齐
                    // （按钮 active 随权威状态机刷新，防左炮/Arm/推弹头卡死）。
                    ApplySnapshot(idx, st, ch, applyState != 0);
                    // ⚠️ 2026-08-15 中途加入：同步药包实体（装填视觉模型，虽只有模型但影响装填机制）——
                    // 刷新 PowderChargeController 分配器/药包显示对齐主机。
                    if (applyState != 0 && g.Powder != null)
                    {
                        try { g.Powder.ApplyInventoryAvailabilityToUI(); } catch { }
                    }
                    string after = "";
                    try { after = g.Reload != null && g.Reload.CurrentState != null ? g.Reload.CurrentState.stateKey : ""; } catch { }
                    if ((++_recvStateLog % 20) == 1)
                        CoopRuntime.LogSource?.LogInfo($"[ReloadSync] recv state idx={idx} st={st} ch={ch} apply={applyState} '{before}' -> '{after}'");
                }
    ...
    private static void HostTick(NetManager net, List<GunReload> guns)
    {
        var w = NetProtocol.Begin(MsgType.ReloadState);
        w.Put((byte)guns.Count);
        w.Put((byte)0); // applyState 标志：0=常规广播（不写 stateIndex，防自动推进；客户端现总是 SetState 对齐）
        // 周期全量广播（开局/漂移兜底）：两端初始装药量/状态差异即使无变化也能在心跳内对齐
        _heartbeat += Interval;
        bool forceHeartbeat = _heartbeat >= HeartbeatInterval;
        if (forceHeartbeat) _heartbeat = 0f;
        bool anyChanged = _forceBroadcast || forceHeartbeat;
        for (int i = 0; i < guns.Count; i++)
        {
            var g = guns[i];
    ...
    private static void ClientTick(NetManager net, List<GunReload> guns)
    {
        for (int i = 0; i < guns.Count; i++)
        {
            ...
            if (st != g.KnownState)
            {
                // 记录本地装填状态变化时间（动画保护窗口基准：本地推进动画进行中不强制 SetState 覆盖）
                g.LastLocalChange = Time.realtimeSinceStartup;
            }
            // ⚠️ 2026-08-23：ch（currentSelectedCharges）变化**不上行**——ch 由 powder select 事件驱动
            // （两端执行相同 OnChargeButtonPressed）自然一致；上行 ch 会与主机权威广播双向覆盖（来回跳）。
            if (st == g.KnownState) continue;
            g.KnownState = st;
            g.KnownCharges = ch;
            try
            {
                ...
                w.Put((byte)g.Index);
                w.Put((byte)Math.Max(st, 0));
                w.Put((byte)0); // ch 不上行（防双向覆盖；主机权威由 powder 事件驱动）
                net.EnqueueBatch(NetProtocol.Snapshot(w), false);
                if ((++_upLog % 20) == 1)
                    CoopRuntime.LogSource?.LogInfo($"[ReloadSync] client up idx={g.Index} st={st}");
            }
    ...
    /// ⚠️ 2026-08-23：currentSelectedCharges **只在 applyState（中途加入）时同步**——常规广播不设 ch，避免
    /// 快照通道 client up ch ↔ host recv ch 双向覆盖（两端 currentSelectedCharges 过渡值来回跳）。ch 由
    /// powder select 事件（两端执行相同 OnChargeButtonPressed）驱动自然一致。
    ...
    private static void ApplySnapshot(int idx, int stateIndex, int charges, bool applyState)
    {
        ...
        try
        {
            if (applyState && g.Powder != null && g.Powder.currentSelectedCharges != charges)
            {
            ...
            // ⚠️ 2026-08-23 回退：常规广播**不 SetState**（恢复 0.1.6）——上一版"常规也对齐"导致主机广播领先 st 时
            // 客户端被 SetState 直接跳到下一阶段（如 st=4 推弹中直接跳 5 选药）→ **跳过推弹头动画/过程**（严重 bug）。
            // 状态机靠**事件驱动**推进（推弹头/选药/下药事件，两端都执行完整过程），快照只在中途加入（applyState=1）时对齐。
            if (applyState && g.Reload != null)
            {
                try
                {
                    ...
                    {
                        // ⚠️ 2026-08-16 动画保护：常规广播（applyState=false）且客户端本地刚推进（窗口 0.8s）
                        // 跳过 SetState——否则主机广播旧值把客户端装填动画拉回 → 动画回跳/不同步。
                        // 中途加入（applyState=true）或超过窗口仍不一致 → SetState 对齐（防按钮卡死）。
                        // ⚠️ 2026-08-23 延迟容忍：本地已推进（CurrentStateIndex > 广播值）→ **不回退**。
                        // 主机广播旧 st 是延迟/时序滞后（客户端推弹头/下药事件已本地推进，reliable 上行还没到主机，
                        // 主机广播的还是旧值）→ SetState 会把本地推进拉回 → 后续按钮（Charge Rammer 等）inactive 锁定。
                        // 中途加入（applyState）仍强制对齐。
                        bool localAhead = !applyState && stateIndex < g.Reload.CurrentStateIndex;
                        bool protectAnim = !applyState && (Time.realtimeSinceStartup - g.LastLocalChange) < 2.0f;
                        if (localAhead)
                        {
                            // 本地已前进：不回退，只刷新按钮 active（不打断本地推进）
                        ...
                        else if (!protectAnim)
                        {
                            ...
                            // ⚠️ 2026-08-15：SetState(force) 只改 currentStateIndex，不触发游戏 OnStateChanged 回调
                            // → 所有依赖装填状态的按钮 active 不刷新（推弹头 "Universal Button Load shell Rammer"、
                            // CylinderShellSelector 弹舱按钮等）→ 客户端按钮 inactive → 点击排队卡死。
                            // 触发 OnStateChanged 让游戏所有监听者（弹舱等）刷新按钮 active。
                            try { g.Reload.OnStateChanged?.Invoke(g.Reload.CurrentState); } catch { }
                            // 兜底：直接刷新 CylinderShellSelector 弹舱按钮（若未注册 OnStateChanged）
                            try
                            {
                                var selAll = UnityEngine.Object.FindObjectsOfType<CylinderShellSelector>();
                                if (selAll != null)
                                    foreach (var sel in selAll)
                                    {
                                        if (sel == null || sel.artilleryReloadController == null) continue;
                                        if (sel.artilleryReloadController.Pointer == g.Reload.Pointer)
                                        {
                                            try { sel.HandleReloadStateChanged(g.Reload.CurrentState); } catch { }
                                            break;
                                        }
                                    }
                            }
                            catch { }
                            CoopRuntime.LogSource?.LogInfo($"[ReloadSync] host-align SetState idx={idx} -> {stateIndex}");
                        }
                        else
                        {
                            // 动画保护窗口：不 SetState 覆盖动画，但**始终刷新按钮 active**（UpdateAllAdvanceButtons
                            // 只按当前 stateIndex 刷新，不推进状态/不打断动画）——防装药拉杆/推弹头按钮 inactive 卡住。
                            try { g.Reload.UpdateAllAdvanceButtons(); } catch { }
                        }
                    }
                }
    ...
    internal static List<GunReload> ResolveGuns()
    {
        var turret = TurretController.Instance;
        if (turret == null)
        {
        ...
        _statesDumped = true;
        _powderDumped = true;
        // ⚠️ 2026-08-17：装填场景就绪（ResolveGuns 首次非空）时立即触发 ControlSync Rescan——
        // 装填 Charge Dial（Magazine Selection）靠 30s 周期 Rescan 才补注册（ON-ENABLE-MISS），
        // 注册前两端 Dial 不同步。场景就绪立即注册 → Charge Dial 位置两端对齐。
        try { ControlSync.ForceRescanNow(); } catch { }
        return _guns;
    }
}
