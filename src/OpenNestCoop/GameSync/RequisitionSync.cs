...
public static class RequisitionSync
{
            ...
            CoopSyncRegistry.RegisterInt("req/powder/stock",
                ...
                v =>
                {
                    ...
                    try
                    {
                        ...
                        // ⚠️ 2026-08-15：库存变化 → 刷新装药按钮 UI（active/指示灯）。
                        // 装药按钮（Button Dispencer/Charge Rammer）的 active/指示灯由
    ...
    private static MissionStatsTracker GetStatsTracker()
    {
        ...
        try
        {
            ...
            if (_statsTracker == null)
                CoopRuntime.LogSource?.LogWarning("[Requisition] MissionStatsTracker.Instance is null (mission scene not loaded)");
        }
    ...
    /// <summary>库存变化后刷新所有装药控制器按钮 UI（active/指示灯）：ApplyInventoryAvailabilityToUI()
    /// 依据库存更新 Button Dispencer/Charge Rammer 的可选/指示灯状态——库存同步后必须刷新，
    /// 否则客机端按钮状态两端不一致（"第五个指示灯黑/装药拉杆无法激活"根因，2026-08-15）。</summary>
    private static void RefreshPowderUI()
    {
        try
        {
            var pcs = UnityEngine.Object.FindObjectsOfType<PowderChargeController>();
            if (pcs == null) return;
            foreach (var pc in pcs)
            {
                if (pc == null) continue;
                try { pc.ApplyInventoryAvailabilityToUI(); } catch { }
            }
        }
        catch { }
    }
