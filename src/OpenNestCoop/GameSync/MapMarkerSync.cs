...
public sealed class MapMarkerSync : ISyncedModule
{
    ...
    public static void EnsureHook()
    {
        ...
        try
        {
            ...
            CoopRuntime.LogSource?.LogInfo("[MapMarkerSync] subscribed MapMarkerPlacer.OnMarkerFinalized");
        }
        ...
        try
        {
            ...
            CoopRuntime.LogSource?.LogInfo("[MapMarkerSync] patched MapMarkerLineUI.UpdateLine (live drag sync)");
        }
    ...
    private static void ApplyMarker(string id, string kind, Vector2 origin, Vector2 target)
    {
        ...
        try
        {
            ...
            {
                CoopRuntime.LogSource?.LogWarning("[MapMarkerSync] map table not ready");
                return;
            }
            var prefab = ResolvePrefab(kind, placer);
            if (prefab == null)
            {
                CoopRuntime.LogSource?.LogWarning($"[MapMarkerSync] no matching prefab: {kind}");
                return;
            }
            ...
            if (ui == null || rt == null)
            {
                CoopRuntime.LogSource?.LogWarning($"[MapMarkerSync] prefab is not MapMarkerLineUI: {kind}");
