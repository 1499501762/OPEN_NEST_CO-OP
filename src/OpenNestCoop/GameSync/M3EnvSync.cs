...
public static class M3EnvSync
{
    ...
    public static void Register()
    {
        ...
        try
        {
            ...
            // 反炮兵永久停止（状态结束，主机权威，客户端只接收）——
            // 仅同步 running/time 不够：主机端 PermanentlyStop（目标全灭/任务结束）后客户端计时器
            // 仍在运行/未结束 → “反炮兵状态结束不同步”。客户端收到 stopped=true 调 PermanentlyStop 对齐。
            CoopSyncRegistry.RegisterBool("env/cbattery/stopped",
                () => GetTimer()?.IsPermanentlyStopped ?? false,
                v =>
                {
                    var t = GetTimer();
                    if (t == null) return;
                    try { if (v && !t.IsPermanentlyStopped) t.PermanentlyStop(); }
                    catch (Exception ex) { CoopRuntime.LogSource?.LogWarning($"M3EnvSync cbstop: {ex.Message}"); }
                }).ClientNoSend = true;
        }
