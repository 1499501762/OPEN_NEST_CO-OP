...
public sealed class ButtonClickSync : ISyncedModule
{
    ...
    private void PollToggleStates(NetManager net)
    {
        ...
        foreach (var t in targets)
        {
            // ⚠️ 2026-08-23：装填按钮（Charge Rammer/Button Dispencer）**不轮询**——点击走 ReloadSync 事件解耦
            // （ShouldTrack=false）。轮询 SetBool 会**清除 Button Dispencer 选中高亮（toggler 1→0）**→
            // PowderChargeController 认为选药量被重置 → Charge Rammer 变 inactive → 拉杆锁定不可交互（根因）。
            // 装填按钮视觉（拉杆拉下/选中高亮）由 ReloadSync 模拟点击动画自然驱动。
    ...
    public static void OnLocalClick(LookAtTarget t)
    {
        try
        {
            ...
            if (net.State != SessionState.Hosting && net.State != SessionState.Joined) return;
            bool tracked = ShouldTrack(t);
            // ⚠️ 2026-08-23 诊断：下装药拉杆（Charge Rammer）点击/active 专属日志（不节流）——确认是
            // "点不动（inactive）"还是"点了不推进状态机"（OnLoadChargesPressed 从未被调用）。
            string tNm = "";
            try { if (t.gameObject != null) tNm = t.gameObject.name; } catch { }
            if (tNm.IndexOf("Charge Rammer", StringComparison.OrdinalIgnoreCase) >= 0)
                CoopRuntime.LogSource?.LogInfo($"[ButtonClickSync] ChargeRammer down active={t.isActive} tracked={tracked}");
            if ((++_clickDiag % 100) == 1)
            {
                string nm = "?";
                try { if (t.gameObject != null) nm = t.gameObject.name; } catch { }
    ...
    private static bool ShouldTrack(LookAtTarget t)
    {
        try
        {
            ...
            // 弹舱动作按钮（Load shell Rammer 推弹头 / Move Cylinder 切弹舱）：已由 CylinderActionSync 事件解耦
            // （直接调 CylinderShellSelector.OnLoadButtonClicked/OnMoveButtonClicked）——不走本层点击复现，
            // 否则对端"点击复现 + 方法调用"双重触发（推弹/切弹两次）。
            if (nm.IndexOf("Load shell Rammer", StringComparison.OrdinalIgnoreCase) >= 0
                || nm.IndexOf("Move Cylinder", StringComparison.OrdinalIgnoreCase) >= 0) return false;
            // 装药按钮（Button Dispencer 选药量 / Charge Rammer 投放发射药）：由 ReloadSync PowderEvent
            // （patch PowderChargeController.OnChargeButtonPressed/OnLoadChargesPressed）事件解耦——不走本层
            // 点击复现，否则"点击复现 + 方法调用"双重触发（选药/投放两次）+ 复现时按钮 inactive 排队刷屏干扰
            // （2026-08-23：Button Dispencer/Charge Rammer 被 Dispenc/Rammer 关键词误命中 → 双路径冲突，
    ...
    private static void ApplyClick(string id, List<bool> togglerStates)
    {
        ...
        foreach (var t in targets)
        {
            ...
            try
            {
                ...
                // toggle 型按钮：对齐最终 bool 状态（防双端互操作回跳，如 Requisition Console/Universal Button
                // after=[1,0,0,0] want=[0,0,0,1] 反复回跳）。单 toggler 或全部 delay=0（瞬时切换）→ SetBool 对齐；
                // 多 toggler 含长动画（楼梯盖板 delay>0）跳过（防打断动画卡中间态）。
                if (togglerStates != null && togglerStates.Count >= 1)
                {
                    var tg = t.GetComponents<AnimatorBoolToggler>();
                    if (tg != null && tg.Length > 0)
                    {
                        try
                        {
                            ...
                            for (int ti = 0; ti < tg.Length && ti < togglerStates.Count; ti++)
                            {
                                try { if (tg[ti].delay > 0f) { allInstant = false; break; } } catch { }
                            }
                            if (allInstant)
                            {
                                bool anyDiff = false;
                                for (int ti = 0; ti < tg.Length && ti < togglerStates.Count; ti++)
                                {
                                    bool want = togglerStates[ti];
                                    bool have;
                                    try { have = tg[ti].GetBool(); } catch { have = want; }
                                    if (have != want) { anyDiff = true; break; }
                                }
                                if (anyDiff)
                                {
                                    for (int ti = 0; ti < tg.Length && ti < togglerStates.Count; ti++)
                                    {
                                        try { tg[ti].SetBool(togglerStates[ti]); } catch { }
                                    }
                                    CoopRuntime.LogSource?.LogInfo($"[ButtonClickSync] toggle align multi '{id}' n={Math.Min(tg.Length, togglerStates.Count)}");
                                }
                            }
                        }
