...
public sealed class ButtonClickSync : ISyncedModule
{
    ...
    public void Tick(float dt)
    {
        ...
        if (_pendingClicks.Count > 0)
        {
            for (int i = _pendingClicks.Count - 1; i >= 0; i--)
            {
                ...
                else if (now - p.ReadyAt > PendingTimeout)
                {
                    ...
                    _pendingClicks.RemoveAt(i);
                    CoopRuntime.LogSource?.LogInfo($"[ButtonClickSync] click dropped (timeout) '{p.Id}'");
                }
            }
        }
        if ((++_log % 300) == 1)
        {
            ...
            try
            {
                ...
                _diagCount = i;
                CoopRuntime.LogSource?.LogInfo($"[ButtonClickSync] tracked total={_diagCount} names=[{names}]");
                if (untrackedUniversal.Length > 0)
                    CoopRuntime.LogSource?.LogInfo($"[ButtonClickSync] UNTRACKED universal/button paths=[{untrackedUniversal}]");
            }
            catch { }
        }
    }
    ...
    private void PollToggleStates(NetManager net)
    {
        ...
        foreach (var (id, states) in changed)
        {
            ...
            CoopRuntime.LogSource?.LogInfo($"[ButtonClickSync] toggle-state '{id}' ={string.Join(",", states.ConvertAll(x => x ? "1" : "0"))}");
        }
    }
