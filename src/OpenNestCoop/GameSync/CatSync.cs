...
public sealed class CatSync : ISyncedModule
{
    ...
    // A2: CatController 实例缓存。猫基本是任务场景静态对象，缓存 + 3s 刷新 + 场景切换即时刷新，
    // 避免每 1/3s 多次 FindObjectsOfType 全场景扫描卡顿。缓存数组顺序 = FindObjectsOfType 顺序
    // （猫索引跨端一致依赖该顺序，刷新用同一 API 保证不变）。已销毁对象为 Unity fake null，调用方过滤。
    ...
    /// <summary>A2: 取猫实例缓存。场景切换即时刷新；定时（3s）兜底捕捉动态生成/销毁；
    /// 未初始化首次扫描。缓存顺序 = FindObjectsOfType 顺序（跨端索引一致依赖）。</summary>
    private static CatController[] GetCats()
    {
        ...
        if (_catCacheTimer >= 3f || _catCache == null)
        {
            _catCacheTimer = 0f;
            return RefreshCats();
        }
    ...
    private static CatController[] RefreshCats()
    {
        _catCache = UnityEngine.Object.FindObjectsOfType<CatController>();
        return _catCache;
    }
    ...
    private void HostBroadcastState(NetManager net)
    {
        try
        {
            ...
            {
                if ((++_sendLog % 30) == 1)
                    CoopRuntime.LogSource?.LogInfo("[CatSync] CatController not found");
                return;
            }
        ...
        foreach (var kv in _hostStates)
        {
            ...
            if (dist > HardSyncDist)
            {
                try
                {
                    ...
                    CoopRuntime.LogSource?.LogInfo($"[CatSync] hard-sync warp dist={dist:0.00} state={(int)c.CurrentState} cat='{c.gameObject?.name}'");
                }
                catch { }
            }
        }
        ...
        try
        {
            switch (ev)
            {
                case 1: // 拾起
                    ...
                    CoopRuntime.LogSource?.LogInfo("[CatSync] apply pick up");
                    break;
                case 2: // 放下
                    try { c.StopCarrying(); } catch { }
                    try { c.transform.position = pos; c.transform.rotation = Quaternion.Euler(0f, yaw, 0f); } catch { }
                    CoopRuntime.LogSource?.LogInfo("[CatSync] apply drop");
                    break;
                case 3: // 驱赶
                    try { c.ShooCat(false); } catch { }
                    CoopRuntime.LogSource?.LogInfo("[CatSync] apply shoo");
                    break;
                case 4: // 抚摸
                    try { c.PetTheCat(); } catch { }
                    CoopRuntime.LogSource?.LogInfo("[CatSync] apply pet");
                    break;
                case 5: // 打断
                    try { c.InterruptCat(); } catch { }
                    CoopRuntime.LogSource?.LogInfo("[CatSync] apply interrupt");
                    break;
            }
        }
