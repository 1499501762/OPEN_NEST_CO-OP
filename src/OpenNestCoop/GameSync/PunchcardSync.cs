        ...
        try
        {
            ...
            try
            {
                for (int i = 0; i < n; i++)
                {
                    ...
                    // ⚠️ 2026-08-22 修复：客机端非槽位卡牌**未拖动时也应用主机位置**——
                    // 原 `if (!net.IsHost && inSlot != 1) continue` 让客机端所有非槽位卡牌都跳过主机位置，
                    // 导致"主机取下卡牌（回牌堆）→ 客机端卡牌位置不跟随"（取下不同步根因）。
                    // 现在只跳过"本端正在拖动"的卡牌（保留客机拖拽本地权威），其余（含主机操作回牌堆）都跟随主机。
                    try
                    {
