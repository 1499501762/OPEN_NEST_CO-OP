...
/// 如需精确同步 Animator 参数或骨骼变换，请用 CoopSyncRegistry/ISyncedModule 注册为值绑定（见 API.md）。
/// </summary>
...
/// Position/Yaw 由 PlayerSync 插值后写入；Speed/Moving 供走路动画；Action/DeviceId 供设备/任务动画。
/// </summary>
...
public interface IPlayerVisualProvider
{
    ...
    /// <summary>销毁视觉（root 由 PlayerSync 统一销毁，这里可清理子对象/资源）。</summary>
