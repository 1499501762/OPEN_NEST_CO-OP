# Open Nest Co-op 架构与技术分析（2026-08-15）
...
Plugin.cs / MelonModEntry.cs        ← 平台壳（仅初始化 + 卸载）
  └─ CoopRuntime.Initialize/Startup/Shutdown
       ...
       ├─ CoopSyncRegistry   ISyncedModule 注册表（~21 个同步模块 + 附加类型路由）
       ├─ StateSnapshotSync  中途加入全量快照（30/31）
       ├─ Patches/Harmony    游戏方法挂钩（开火/输入/装填/地图等）
       └─ GameSync/*         ~30 个同步模块（Turret/Player/Value/Control/Entity/Cat...）
```
...
已注册：mission/hatch/sequence/mapmarker/maptoken/recordplayer/button。
...
| ValueSync | 变化检测 + 2s 心跳 + 拖拽 settle + 插值 | 0.2s / 高频 0.033s(Lever) | 双向 |
| ControlSync | 场景扫描注册 → 委托 ValueSync | Rescan 3s | — |
| TurretSync | **纯事件**（开火广播），无状态同步 | 事件触发 | 主机→全员 |
| EntitySync | 状态广播（聚合 n>1）| 0.5s | 双向 |
| CatSync | 主机 AI 软同步 + 交互事件 + 偏差硬同步 | 1/3s | 双向 |
| ButtonClickSync | 点击事件 + toggle 轮询 | 0.8s 轮询 | 双向 |
...
**TurretSync**（架构最精简）：**不做状态同步**——Lever/Gear 位置一致 → 游戏确定性逻辑 → 炮塔自然一致。
只同步开火事件（`FireShell` Harmony postfix → 广播 GunFire → 对端 `RequestFire` 复现，防环）。
...
- **B1 `ControlSync.Rescan` 每 3s 4 次 `FindObjectsOfTypeAll`**（:147,150,188,225）⭐ 高 ✅ 已实施（2026-08 轨道 A）
  → **并行方案**：OnEnable 逐控件注册（主路径，Harmony patch DialInteractable/LinearSliderInteractable/SliderEnergyMomentumSpinner OnEnable → `OnControlEnabled` 即时注册单个控件，不触发全量扫描）
    + Rescan 保底（5s 周期 + 场景 buildIndex 变化初始化；`detectMiss` 区分——初始化扫描不算漏，运行期保底记录漏检）
