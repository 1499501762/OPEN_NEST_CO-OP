...
>
> **进度（2026-08-23）**：V2 分层 **M1-M7 已基本完成**——`SyncV2Bootstrap.RegisterAll()` 已注册全部模块：
> HostDataLayer(203) / ValueLayer(201) / EventLayer(200) / ButtonLayer(202) / ControlSyncV2(204 占位) /
> PlayerSyncV2(205) / EntitySyncV2(206) / CatSyncV2(207) / RecordPlayerSyncV2(208) / ReloadSyncV2(209·210·214) /
> ReconPhotoSyncV2(215) / CoffeeSyncV2(216) / CounterBatterySyncV2(217) / RecordItemSyncV2(218) / ShellSyncV2(219) /
> SequenceSyncV2(220) / HatchSyncV2(221) / MapTokenSyncV2(222) / PurchaseSyncV2(223) / PunchcardSyncV2(224·225) /
> MapMarkerSyncV2(226) / MissionSyncV2(227) / TeleprinterSyncV2(228) / GunLinkSyncV2(229) / M3EnvSyncV2 / RequisitionSyncV2。
> 剩余 **M8 收尾**：`--sync old` 与 `--sync new` 双方案独立可跑、V2 全绿（见 §5）。
...
- 首字节 `MsgType`。现有分配：**会话 1-6、内建同步 10-32、Batch=120、自定义模块 100-143**
  （100 Coffee / 102 Mission / 103 CounterBattery / 104 Entity / 105 ReconPhoto / 106 Cat /
  107 MapMarker / 108 RecordItem / 109 Shell / 110 Sequence / 117 Hatch / 118 ButtonClick /
  119 MapToken / 121 GunLink / 130 MissionEvent / 131 Notification / 132 Purchase / 134 Teleprinter /
  136 Punchcard / 140 Arm / 141 CylinderAction / 142 ChargeInventory / 143 ChargeButton）。
...
**✅ 已实现**（`AutoJoin.ParseCommandLine()` 解析 `--sync old|new`，默认 old）：
```csharp
// AutoJoin.cs
...
if (OpenNestCoop.Net.AutoJoin.WantNewSync)
    SyncV2.SyncV2Bootstrap.RegisterAll();   // 注册 V2 分层模块（M1-M7 全部）
else
    RegisterLegacyModules();                // 现有 CoopSyncRegistry.RegisterModule(...) 全部（V1）
```
- 默认 old（稳定发布线）；显式 `--sync new` 走新方案。
- **握手校验**：Hello/Welcome 带 `syncScheme`（0=old 1=new，见 `SyncV2Bootstrap.SchemeId`），
  两端方案不一致时拒绝（避免协议错乱）。
- `scripts\dualtest.ps1 -Sync new` 可把 `--sync new` 同时传给两端做双开验证。

### 3.3 SyncV2 目录与接口（已实现）

```
src\OpenNestCoop\SyncV2\
  SyncV2Bootstrap.cs      RegisterAll()：注册所有 V2 模块（MsgType ≥200）
  HostDataLayer.cs        IRoleAuthority/IHostStore + 主机权威数据层实现（MsgType=203，全量快照）
  EventLayer.cs           EventLayer（泛型事件广播+对端复现+防环，MsgType=200）
  ValueLayer.cs           ValueLayer（值同步，MsgType=201）
  ButtonLayer.cs          ButtonLayer（按钮 toggle 状态，MsgType=202）
  ControlSyncV2.cs        控件发现层（204 占位，无自身消息；场景加载事件驱动 + 12s 低频兜底）
  PlayerSyncV2.cs / EntitySyncV2.cs / CatSyncV2.cs / RecordPlayerSyncV2.cs / ReloadSyncV2.cs
  ReconPhotoSyncV2.cs / CoffeeSyncV2.cs / CounterBatterySyncV2.cs / RecordItemSyncV2.cs / ShellSyncV2.cs
  SequenceSyncV2.cs / HatchSyncV2.cs / MapTokenSyncV2.cs / PurchaseSyncV2.cs / PunchcardSyncV2.cs
  MapMarkerSyncV2.cs / MissionSyncV2.cs / TeleprinterSyncV2.cs / GunLinkSyncV2.cs / M3EnvSyncV2.cs / RequisitionSyncV2.cs
  MissionEventSyncV2.cs / NotificationSyncV2.cs   （无自身 MsgType，实例化即注册复现回调，走 EventLayer 200）
```

**实际接口（已落地）**：
```csharp
// 主机能力抽象：谁是主机 / 谁可写 / 主机 id
public interface IRoleAuthority { bool IsHost { get; } bool IsOnline { get; } ulong HostId { get; } }

// 主机权威数据层接口：其他层统一经此读写共享状态 + 广播
public interface IHostStore : IRoleAuthority
{
    float GetFloat(string id); bool GetBool(string id); int GetInt(string id);       // 读（主/客通用）
    void SetFloat(string id, float v); void SetBool(string id, bool v); void SetInt(string id, int v); // 仅主机有效（客机被忽略）
    void ApplyFloat(string id, float v); void ApplyBool(string id, bool v); void ApplyInt(string id, int v); // 远端值应用（自动防环）
    void Broadcast(byte msgType, Action<NetDataWriter> write, bool reliable);       // 主机→全员 / 客机→主机中继
    void SendToHost(byte msgType, Action<NetDataWriter> write, bool reliable);      // 客机→主机定向
}

// 权威模型：谁操作谁权威（默认，本地立即执行+广播+心跳对齐）/ 主机权威（仅主机广播，客机只接收）
public enum V2Authority { Operator, Host }
```

每个层/模块都实现 `ISyncedModule`（MsgType ≥200），注册进 `CoopSyncRegistry` 走同一套 Tick/路由/会话生命周期。
迁移已完成并**吸收** `docs/ARCHITECTURE.md` 对应优化项（如 ControlSync 3s 全场景扫描 → 场景加载事件驱动 + 12s 低频兜底）。
...
| # | 任务 | 产出 | 完成标准 |
|---|------|------|---------|
| M1 | ✅ 完成 | `SyncV2/` 目录 + `SyncV2Bootstrap` + `--sync` 参数 | 双端 `--sync new` 能启动不崩、走 V2 注册 |
| M2 | ✅ 完成 | `HostDataLayer.cs`（MsgType=203） | 主机读写 + 广播可被其他层调用 |
| M3 | ✅ 完成 | `ValueLayer.cs`（MsgType=201） | ValueSync 的 Binding 迁入；Operator/Host 双权威、双端值一致 |
| M4 | ✅ 完成 | `EventLayer.cs`（MsgType=200） | 开火/点击事件跨端复现 + 防环（_reproducing id 级） |
| M5 | ✅ 完成 | `ButtonLayer.cs`（MsgType=202） | 按钮 toggle 状态跨端一致 |
| M6 | ✅ 完成 | `ControlSyncV2`（204 占位） | 控件发现场景加载事件驱动 + 12s 低频兜底，无 3s 全场景扫描 |
| M7 | ✅ 基本完成 | 其余模块全部迁入（见 §3.3 清单） | PlayerSyncV2 205 / EntitySyncV2 206 / CatSyncV2 207 / ... / GunLinkSyncV2 229 / M3EnvSyncV2 / RequisitionSyncV2 |
| M8 | ⬜ 收尾 | — | `--sync old` 与 `--sync new` 都可独立跑、V2 全绿 |
