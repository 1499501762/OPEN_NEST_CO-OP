...
// ⚠️ 关键：LoadFromStream 返回的 bundle 是延迟加载的——后续 LoadAsset/Instantiate 时 Unity
// 会回调 ManagedStreamSeek 从 Stream 重新读取。因此 FileStream 必须保持打开（进程内缓存，
// 绝不能 Dispose），否则读不到完整数据 → "Mismatched serialization" → 崩溃。
private static Il2CppSystem.IO.FileStream _stream;
...
public bool TryLoad()
{
    ...
    try
    {
        string path = FindBundlePath();          // 找 Models/player.bundle 或环境变量 ONC_BUNDLE
        if (path == null) return false;

        string full = Path.GetFullPath(path);
        // ✅ 唯一可用入口：Il2CppSystem.IO.FileStream 必须保持打开
        _stream = new Il2CppSystem.IO.FileStream(
            full, Il2CppSystem.IO.FileMode.Open,
            Il2CppSystem.IO.FileAccess.Read, Il2CppSystem.IO.FileShare.Read);
        _bundle = AssetBundle.LoadFromStream(_stream);
        if (_bundle != null)
        {
            _prefab = LoadPrefab(_bundle);       // LoadAsset<GameObject>("Player")
...
`PlayerSync.ResolveProvider` 中 `animatorBundleEnabled = true`：
- `TryLoad()` 成功 → 用 `AnimatorAvatarVisualProvider`；
- 失败 → 回退 `ExternalModelProvider`（glb 自采样，保底）。
...
- [ ] bundle 内存管理（会话结束 `Unload(false)` 释放；注意先停止引用）
