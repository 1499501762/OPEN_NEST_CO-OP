...
| **ShellSync** | 变化检测 | ⚠️ 缺失 |
| **ButtonClickSync** | 点击事件 + toggle 轮询 | ✅ toggle 状态已纳入 `button` 快照（指示灯/楼梯盖板）；纯点击瞬时动作无需快照 |
...
- 已注册模块（8 个）：
  ...
  - `button` → `ButtonClickSync.BuildButtonSnapshot` / `ApplyButtonSnapshot`（指示灯/楼梯盖板等多 toggler 按钮 toggle 状态对齐）
  - `entity` → `EntitySync.BuildEntitySnapshot` / `ApplyEntitySnapshot`（反炮兵炮兵/药包等任务动态实体中途加入对齐）
...
- **不需要快照**：ValueSync/CatSync/RecordItemSync/CoffeeSync/GunLinkSync（周期全量自动对齐）、ShellSync（1.5s 心跳）、MissionEventSync/PurchaseSync（纯事件，无状态可回放）
  ⚠️ **核对（2026-08-23）**：ButtonClickSync 的 **toggle 状态已纳入 `button` 快照**（指示灯/楼梯盖板等多 toggler
  按钮中途加入对齐），仅纯点击瞬时动作无需快照；EntitySync 除周期全量外也已注册 `entity` 快照（动态实体对齐）。
...
2. **按钮/事件类**：纯点击瞬时动作无需快照（不可回放的历史动作）；ButtonClickSync 的 **toggle 状态需快照**（已纳入 `button`）
3. **带宽**：全量状态只在加入时发一次，影响小
4. **角色分配**：新成员加入需分配岗位（Role），现有 `SetRole` 已支持
5. **ValueSync 心跳已覆盖**：手动触发一次心跳即可，不需额外快照

## 验收标准
- 中途加入后：能看到当前任务（若在任务中）、炮塔/控件状态、装填状态、地图标记/令牌、任务实体、猫、唱片机、
  **按钮 toggle 状态（指示灯/楼梯盖板）**，与主机一致
- 加入不打断现有成员的同步
- 双平台（BepInEx + MelonLoader）均可

> **状态（2026-08-23）**：方案 B 已实现并启用——`StateSnapshotSync`（MsgType=30）统一容器，已注册 **8 个快照**
> （mission/hatch/sequence/mapmarker/maptoken/recordplayer/button/entity）。
