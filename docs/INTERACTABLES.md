...
| 版本 | 状态 | 说明 |
|---|---|---|
| **2026-09-12n（未发版）** | 🔄 待复测 | **界面文案全部改语言键 + 外部语言键文件**：新增 `Core/Loc/LocFile.cs`（读/生成/2s 热重载）+ `Core/Loc/LocDefaults.cs`（内置默认表，206 键）；`UI/CoopLoc.cs` 只留键，文本来自 `OpenNestCoop.lang.ini`（BepInEx/config 或 UserData，与配置同目录）；错误/拒绝原因改为 `@键` 下发、**接收端按本端语言**显示。另：选项卡/列表的 `[锁]`、`旧版?`、F10 工具文案均上键。见 `docs/LOCALIZATION.md`（含实测：首启生成 / `language = en` 检测 / 4s 内热重载） |
| **2026-09-12m（未发版）** | 🔄 待复测 | **局域网联机（LAN Co-op）**：新增 `LanTransport`（TCP 多客户端）+ `LanDiscovery`（UDP 广播）+ `Core/Identity`（**SteamID 优先，否则 FakeID 自动生成并写入配置**）；菜单新增**独立选项卡**（Steam 大厅 / 局域网联机），局域网面板：自定义用户名、本机 IP 一键复制、主机 IP 粘贴（支持 `ip:port` 自动拆分）、扫描列表；**同 Steam 账号双开自动派生唯一身份**。另：选项卡选中标记改用纯 ASCII（原 `▸` 字体缺字显示方框）、聊天提示去 emoji。已 CLI 双开实测连通。详见 `docs/LAN.md` |
| **2026-09-12l（未发版）** | 🔄 待复测 | **铁巢/炮弹起点图标同步改为「变化检测 + 缓存」（用户要求）**：`NestSync`（146）不再 1.5s 无条件重发——0.5s 只做本地读值 → 与缓存比较，**变了才发**（主客无变化时零包）；客机仅在「本端图标与缓存值不一致」（尚未写入/被本地改动/晚生成）时才补写。因不再有周期重发兜底：146 已用新 API `RegisterCriticalType` 登记为**关键包**（合包满不丢）+ 新增 `OnLateJoin` 中途加入补发一次。日志：`[NestSync] client icon apply (re-assert)` / `host late-join resend to ...`（已路由到 sync.log） |
