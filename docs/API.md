...
- **主机权威**：主机每 0.2s 轮询所有注册值，变化时广播 `ControlState`（`kind+id+value`）。
- **客户端**：本地值变化上行 `ControlCmd` → 主机应用后转发。
- 内建防环 + 插值，无需扩展方处理。
...
w.Put(value);
Plugin.Net.Transport.Send(Plugin.Net.HostSteamId, NetProtocol.Snapshot(w), reliable: true);
```

`NetProtocol.Begin(type)` 写入类型头；`NetProtocol.Snapshot(w)` 取出完整字节数组；`Plugin.Net.Transport.Send(steamId, data, reliable)` 发送。
...
`AvatarPose.Action` 目前由 `PlayerSync` 推断（`Moving`/`Idle`）。如需同步自定义动作（装填/操作设备），
可扩展 `PlayerPos` 协议（加 `action + deviceId` 字段），或用独立 `ISyncedModule` 发送动作状态。
...
| 16 | `PlayerPos` | 客户端→主机→其他 | 玩家位置/朝向 |
...
| 100 | `CoffeeSync` | 咖啡机 | BrewState（内置示例模块） |
| **100+** | 自定义 | — | 留给 `ISyncedModule` |
...
- **消息类型冲突？** 内建占用 1-27，自定义模块用 **100+**。
- **插值会不会和本地操作打架？** `busy` 返回 true（如正在拖拽）时自动放弃远端覆盖，本地优先。
- **角色没有动画素材？** 默认 `DefaultPlayerVisualProvider` 已内置；自定义实现里用 pose 驱动你自己的 Animator 即可。
