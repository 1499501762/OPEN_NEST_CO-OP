...
>
> **进度（2026-08-23）**：轨道 A 基本完成（A0-A3 已验证）；轨道 B 的 B0-B2（里程碑 M1-M7）已完成，
> 剩余 B3/M8 收尾（双方案独立可跑、V2 全绿）。

---

## 轨道 A：旧方案优化（当前 AI 负责）

### A0. Feature Flag 骨架（先做，V2 的挂载点）——✅ 完成
- [x] `AutoJoin.ParseCommandLine()` 加 `--sync old|new`（默认 old，字段 `WantNewSync`）
- [x] `CoopRuntime.Startup()` 按方案分支：`--sync new` → `SyncV2Bootstrap.RegisterAll()`；否则现有模块注册
- [x] Hello/Welcome 握手带 `syncScheme`（`SyncV2Bootstrap.SchemeId`），两端不一致拒绝
- [x] 双端 `--sync old` 回归无变化；`--sync new` 能启动（V2 全量注册也不崩）
- 验证：✅ `.\scripts\dualtest.ps1 -Local`（默认 old 全绿）；`dualtest.ps1 -Local -Sync new` 走 V2
...
### B0. 骨架（与 A0 对接）——✅ 完成
- [x] `SyncV2/` 目录 + `SyncV2Bootstrap.RegisterAll()`
- [x] 读 `--sync` 参数，`--sync new` 时走 V2 注册
- [x] MsgType ≥200 独立段（200-229，见 NetProtocol.MsgType）

### B1. 分层实现（里程碑 M2-M5）——✅ 全部完成
- [x] `HostDataLayer`：IHostStore/IRoleAuthority（主机读写/授权/广播/全量快照）
- [x] `ValueLayer`（MsgType=201）：值同步（ValueSync 的 Binding 迁入；Operator/Host 双权威）
- [x] `EventLayer`（MsgType=200）：泛型事件广播+复现+防环（_reproducing id 级）
- [x] `ButtonLayer`（MsgType=202）：按钮 toggle 状态

### B2. 模块迁移（里程碑 M6-M7，迁移即吸收优化）——✅ 基本完成
- [x] ControlSync 发现逻辑 → `ControlSyncV2`（吸收 Rescan 扫描优化：场景加载事件驱动 + 12s 低频兜底）
- [x] 逐模块迁移（`SyncV2Bootstrap.RegisterAll()` 已全部注册）：
